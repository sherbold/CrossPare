...
public abstract class AbstractWekaEvaluation implements IEvaluationStrategy {
                      ...
                      List<IResultStorage> storages)
    {
        ...
        while (classifierIter.hasNext()) {
            ...
            result.setSizeTrainingOriginal(traindataOriginal.numInstances());
            result.setBiasTestData(getBias(testdata));
            result.setBiasTrainingData(getBias(traindata));
            result.setBiasTrainingOriginal(getBias(traindataOriginal));
            result.setProp1Defect(!Double.isNaN(getProp1Defects(testdata, efforts))  ? getProp1Defects(testdata, efforts) : -1);
            result.setProp1Clean(!Double.isNaN(getProp1Clean(testdata, efforts))  ? getProp1Clean(testdata, efforts) : -1);
            result.setError(!Double.isNaN(eval.errorRate()) ? eval.errorRate() : -1);
            result.setRecall(!Double.isNaN(eval.recall(1))  ? eval.recall(1) : -1);
            result.setPrecision(!Double.isNaN(eval.precision(1)) ? eval.precision(1) : -1);
            result.setFscore(!Double.isNaN(eval.fMeasure(1)) ? eval.fMeasure(1) : -1);
            result.setGscore(!Double.isNaN(gmeasure) ? gmeasure : -1);
            result.setMcc(!Double.isNaN(eval.matthewsCorrelationCoefficient(1)) ? eval.matthewsCorrelationCoefficient(1) : -1);
            result.setBalance(!Double.isNaN(balance) ? balance : -1);
            result.setAuc(!Double.isNaN(eval.areaUnderROC(1)) ? eval.areaUnderROC(1) : -1);
            result.setAucec(aucec);
            result.setAucAlberg(aucAlberg);
            result.setAucRoI(aucWithRoI);
            result.setNofb20(nofb20);
            result.setRelb20(relb20);
            result.setNofi80(nofi80);
            result.setReli80(reli80);
            result.setRele80(rele80);
            result.setNecm10(necm10);
            ...
            result.setTpr(!Double.isNaN(eval.truePositiveRate(1))  ? eval.truePositiveRate(1) : -1);
            result.setTnr(!Double.isNaN(eval.trueNegativeRate(1))  ? eval.trueNegativeRate(1) : -1);
            result.setFpr(!Double.isNaN(eval.falsePositiveRate(1))  ? eval.falsePositiveRate(1) : -1);
            result.setFnr(!Double.isNaN(eval.falseNegativeRate(1))  ? eval.falseNegativeRate(1) : -1);
